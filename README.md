...
- **考试静默窗口**：考试开始前 30 分钟至结束后 5 分钟内软件全局静音（指定的考试提醒除外），避免影响考场。
...
- **平滑动画**：页面切换采用整页淡入过渡，并给内容容器设置实心主题底色，从机制上杜绝切换时出现黑色背景/残影；过渡期间新页全程真实渲染、结束后立即恢复原生绘制，复杂页（含 QTableView/QScrollArea）不卡顿；侧边栏选中高亮采用几何滑块动画。
- **静音提示**：除考试提醒外，所有信息/警告/确认均改用无系统音效的自绘 Toast 与静音确认框，不打断课堂。
- **全屏广播**：无边框全屏 + 置顶 + 自动隐藏任务栏，进入即自动切到「今日概览」，并配合系统唤醒限制保持屏幕常亮。
- **启动性能**：页面懒加载 + 空闲时预加载设置页 + 依赖延迟加载，降低内存占用与启动时间。
